...
def iterate(pixel):
        ...
        for delta_y in (-1 / 3, 0, 1 / 3):
            ...
            for i in range(julia_set.iterations):
                ...
                if distance > 1_000_000:
                    outside += 1
                    strength += i / julia_set.iterations * (1 - log(distance) * 0.01)
                    break
            else:
                floats.append(-1)
    ...
    return Color(r, g, b) * (outside / 9) + Color(0, 0, 0) * (1 - outside / 9)


if __name__ == "__main__":
    julia_set = Fractal("Julia Set", iterations=50, pixel=True, threads=8)
