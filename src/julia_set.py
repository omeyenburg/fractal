...
def iterate(pixel):
    ...
    for delta_x in (-1/3, 0, 1/3):
        for delta_y in (-1/3, 0, 1/3):
            x = (pixel[0] - julia_set.width / 2 +
                 delta_x) / julia_set.height * 2.5
            y = (pixel[1] - julia_set.height / 2 +
                 delta_y) / julia_set.height * 2.5
            ...
            for i in range(julia_set.iterations):
                ...
                if distance > 1_000_000:
                    floats.append(i / julia_set.iterations *
                                  (1 - log(distance) * 0.01))
                    break
            else:
                floats.append(-1)
    ...
    julia_set.window.set_at(
        ...
        tuple(Color(r, g, b) * (len(floats) / 9) +
              Color(0, 0, 0) * (1 - len(floats) / 9))
    )
